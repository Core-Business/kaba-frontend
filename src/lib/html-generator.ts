...
export function generatePOAHTML(poa: POA): string {
  ...
  const activitiesHtml = poa.activities.map(activity => `
    <div class="activity-item" style="margin-bottom: 15px; padding-left: ${activity.number.includes('.') ? '20px' : '0px'};">
      <h4 style="font-size: 1.1em; margin-bottom: 5px;">Activity ${activity.number}: ${activity.type !== 'individual' ? `(${activity.type})` : ''}</h4>
  ...
  return `
    ...
    <head>
      ...
      <title>${poa.header.title || 'Plan of Action'}</title>
      <style>
    ...
    <body>
      <div class="container">
        <header>
          ${logoHtml}
          <h1>${poa.header.title || 'Plan of Action'}</h1>
          <p><strong>Author:</strong> ${poa.header.author || 'N/A'}</p>
          <p><strong>Version:</strong> ${poa.header.version || 'N/A'}</p>
          <p><strong>Date:</strong> ${poa.header.date ? new Date(poa.header.date).toLocaleDateString() : 'N/A'}</p>
        </header>
